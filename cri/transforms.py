...
def euler2quat(pose_e, axes='sxyz'):
    ...
    """
    pose_e = np.asarray(pose_e, dtype=np.float64).ravel()
...
def quat2euler(pose_q, axes='sxyz'):
    ...
    """
    pose_q = np.asarray(pose_q, dtype=np.float64).ravel()
...
def axangle2quat(pose_a):
    ...
    """
    pose_a = np.asarray(pose_a, dtype=np.float64).ravel()
...
def mat2quat(pose_m):
    ...
    """
    pose_m = np.asarray(pose_m, dtype=np.float64)
